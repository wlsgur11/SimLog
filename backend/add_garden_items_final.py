...
def add_garden_items_final():
    print("실제 존재하는 파일들만 정확히 확인하여 정원 아이템들을 추가합니다...")
    
    # 데이터베이스 세션 생성
    db = Session(engine)
    
    try:
        # 실제 존재하는 파일들만 포함한 상점 아이템들
        shop_items = [
            # 꽃들 (실제 파일 존재 확인)
            {
                'item_type': 'flower',
        ...
        db.commit()
        print("✅ 실제 존재하는 파일들만 사용하는 정원 아이템 추가가 완료되었습니다!")
